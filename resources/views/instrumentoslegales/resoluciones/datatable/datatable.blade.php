            ...
            <tbody>
                @foreach($resoluciones as $resolucion)
                    <tr>
                        ...
                        <td>{{ $resolucion->fecha_importacion }}</td>
                        <td>{{ $resolucion->fecha_aprobacion }}</td>
                        <td>{{ $resolucion->categoria }}</td>
                        
                        <td>
                            <a href="{{ Storage::url('/' . $resolucion->ruta) }}" class="btn btn-success btn-sm" target="_blank">
                                <i class="fas fa-eye"></i> 
                            </a>
                        </td>
                        <td>
                            <button type="button" class="btn btn-info btn-sm" onclick="printPDF('{{ Storage::url('/' . $resolucion->ruta) }}')">
                                <i class="fas fa-print"></i> 
                            </button>
                        </td>
                        <td>
                            <a href="{{ route('resoluciones.download', $resolucion->nombre) }}" class="btn btn-warning btn-sm">
                        ...
                        @if(auth()->user()->can('delete articles'))
                            <td>
                                <form action="{{ route('resoluciones.destroy', $resolucion->nombre) }}" method="POST" style="display:inline;" onsubmit="confirmDelete(event)">
                                    @csrf
                                    @method('DELETE')
